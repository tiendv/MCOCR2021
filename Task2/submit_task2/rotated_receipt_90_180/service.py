...
app = Flask(__name__)
FAKE_ERROR_MESSAGE = "<p style=\"color:red\"><b>\
    ERROR 504! Server encountered an unexpected error \
...
log_path = '/storage/service_log'

@app.route('/rotate-dir', methods=['GET', 'POST'])
def pipeline_dir():
    ...
    if not os.path.exists(dataset_dir): use_mount = True
    output_dir = request.form['output_dir']
    
    if use_mount:
        dataset_dir = '/mount' + dataset_dir
        output_dir = '/mount' + output_dir
    
    print(dataset_dir)
    print(output_dir)
    
    if not os.path.exists(dataset_dir):
        return 'Dataset directory not found!', 600
    
    rotate_dir(dataset_dir, output_dir)
    
    return output_dir


@app.route('/rotate-img', methods=['GET', 'POST'])
def pipeline_img():
    if request.method != 'POST': 
        return FAKE_ERROR_MESSAGE
    print(request.form)
    ...
    if 'img' in request.files:
        ...
        if not os.path.exists(img_path): use_mount = True
        
        if use_mount:
            img_path = '/mount' + img_path
        
        print(img_path)
        
        if not os.path.exists(img_path):
            return 'Image not found in path', 600
